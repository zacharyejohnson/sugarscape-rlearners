...
# import keras
from q_learning import DQN, Transition, PrioritizedReplayBuffer, DoubleDQN, EpsilonGreedy, \
    RewardNormalizer
...
class Agent:
    ...
    def __init__(self, model, row, col, ID, parent=None):
        ...
        def selectParameters(mutate=False,
            ...
            def setReservationDemand():
                ...
                if self.parent == None:
                    ...
                    self.reservation_ratio = np.exp(
                        np.log(min_res_ratio) + np.random.random() * (np.log(max_res_ratio) - np.log(min_res_ratio)))
                    self.reservation_ratio *= (
                                self.model.consumption_rate["water"] / self.model.consumption_rate["sugar"])
                    min_price_change = .5
                    max_price_change = 2
                    self.price_change = np.e ** (np.log(min_price_change) + np.random.random() * (
                                np.log(max_price_change) - np.log(min_price_change)))
            ...
            def setReproductionLevel():
                ...
                if self.parent == None:
                    for good in self.model.goods:
                        ...
                        self.reproduction_ratio[good] = get_lognormal_rand(min_reproduction_ratio[good],
                                                                           max_reproduction_ratio[good])
                else:
                    if mutate_kwargs["reproduction_ratio"]:
                        for good in self.model.goods:
                            mutation = np.log(parent.reproduction_ratio[good] * self.mutate_rate)
                            if random.getrandbits(1) == 1:
                                self.reproduction_ratio[good] = np.exp(
                                    np.log(parent.reproduction_ratio[good]) - mutation)
                            else:
                                self.reproduction_ratio[good] = np.exp(
                                    np.log(parent.reproduction_ratio[good]) + mutation)
                            ...
                            #  self.reproduction_ratio[good] = min_val + np.random.random() * (max_val - min_val)
            ...
            def selectBreed():
                if self.parent != None:
                    ...
                    np.random.shuffle(shuffle_breeds)
            ...
            def setMutateRate():
                if self.model.mutate:
                    if self.parent == None:
                        ...
                        # self.mutate_rate = min_rate + np.random.random() * (max_rate - min_rate)
                    elif random.random() < self.parent.mutate_rate:
                        x = np.random.uniform(2, 50)
                        self.mutate_rate = 1 / x

                    elif mutate_kwargs["mutate_rate"]:
                        if random.getrandbits(1) == 1:
                            self.mutate_rate = np.exp(np.log(self.parent.mutate_rate) - np.log(
                                self.parent.mutate_rate * self.parent.mutate_rate))
                        else:
                            self.mutate_rate = np.exp(np.log(self.parent.mutate_rate) + np.log(
                                self.parent.mutate_rate * self.parent.mutate_rate))
                        # self.mutate_rate = min_rate + np.random.random() * (max_rate - min_rate)
            ...
            def init_nn():
                ...
                if self.parent == None:
                    discount_rate = min(0.999, np.random.normal(scale=0.95, loc=.01))
                    learning_rate = max(0.0001, np.random.normal(scale=0.2, loc=0.075))
                    self.replay_frequency = np.random.randint(1, 5)
                    self.min_res_ratio = 0.2
                    self.max_res_ratio = 5
                    self.num_bins = 100
                    self.replay_memory_length = np.random.randint(1, 5)
                    self.n_step_buffer = []
                    self.n = np.random.randint(1, 3) if self.parent is None else mutateAttr(self.parent.n, True)
                    self.n_layers = np.random.randint(1, 5)  # Genetically determine the number of layers
                    self.layer_size = np.random.randint(4, 128)  # Genetically determine the layer size
                    ...
                    learning_rate = min(0.5, max(0.00000001, mutateAttrRate(self.parent.nn.online_network.learning_rate)))
                    self.replay_frequency = max(1, mutateAttr(self.parent.replay_frequency, integer=True))
                    ...
                    self.replay_memory_length = min(5, max(0, mutateAttr(self.parent.replay_memory_length, integer=True)))
                    self.n_step_buffer = []
                    self.n = mutateAttr(self.parent.n, True)
                    self.n_layers = min(5, max(1, mutateAttr(self.parent.n_layers, integer=True)))
                    self.layer_size = min(128, max(4, mutateAttr(self.parent.layer_size, integer=True)))
            ...
            def get_lognormal_rand(min, max):
                return np.exp(np.log(min) + np.random.random() * (np.log(max) - np.log(min)))
            ...
            setTargets()
            self.vision = 1  # random.randint(1, self.model.max_vision)
        ...
        def setStocks():
            ...
            if self.parent == None:
                for good, vals in self.model.goods_params.items():
                    val = np.random.randint(vals["min"], vals["max"])
        ...
        def mutateAttr(attr, integer=False):
            ...
            if integer:
                ...
                attr_val = np.random.uniform(min_val, max_val)
            return attr_val
        ...
        def mutate():
            ...
            mutate_dict = {key: True if np.random.random() < self.mutate_rate else False for key, val in
                           inheritance.items()}
    ...
    def setBreedBasic(self, herder):
        ...
        random_breed = random.choice(self.model.primary_breeds)
        setattr(self, random_breed, True)
    ...
    def mutateAttr(self, attr, rate):
        ...
        attr_val = min_val + np.random.random() * (max_val - min_val)
    ...
    def selectBreedParameters(self, mutate, parent, herding,
        ...
        def generateBreedParameters():
            if breed == "herder":
                ...
                self.top_wealth = getattr(self.parent, self.herding_metric) if inheritance is not None else getattr(
                    self, self.herding_metric)

            self.MRS = (self.wealth_by_good["water"] / self.model.consumption_rate["water"]) / (
                        self.wealth_by_good["sugar"] / self.model.consumption_rate["sugar"])
    ...
    def defineInheritance(self):
        ...
        # np.random.shuffle(copy_attributes)
    ...
    def updateParams(self, trade=False):
        ...
        def checkReservation():
            ...
            reproduction_ratio = (self.reproduction_criteria["water"] / self.model.consumption_rate["water"]) / (
                        self.reproduction_criteria["sugar"] / self.model.consumption_rate["sugar"])
            adjustment = np.log(reproduction_ratio / ((self.water / self.model.consumption_rate["water"]) / (
                        self.sugar / self.model.consumption_rate["sugar"])))
        ...
        def checkQValuation():
            ...
            reservation_ratio_bins = np.logspace(np.log10(self.min_res_ratio), np.log10(self.max_res_ratio),
                                                 num=self.num_bins)
    ...
    def calculate_z_scores(self, state):
        stored_states = self.memory.get_all_states()
        if len(stored_states) > 1:
            mean = np.mean(stored_states, axis=0)
            std = np.std(stored_states, axis=0)
            return (state - mean) / (std + 1e-8)  # Add small epsilon to avoid division by zero
        else:
            return state  # Return original state if not enough data for z-score

    def remember(self, state, action, reward, next_state):
        self.n_step_buffer.append((state, action, reward, next_state, True))

        if len(self.n_step_buffer) < self.n:
            return

        r = sum([self.nn.online_network.discount_rate ** i * t[2] for i, t in enumerate(self.n_step_buffer)])
    ...
    def q_value_state(self, state):
        normalized_state = self.calculate_z_scores(state)
    ...
    def choose_action(self, state):
        if random.random() < self.epsilon_greedy.get_epsilon():
            return random.randint(0, self.num_bins - 1)
        return self.q_value_state(state)

    # def learn(self, transition):
    #     state, action, reward, next_state = transition
    #     state = self.calculate_z_scores(state)
    #     next_state = self.calculate_z_scores(next_state)
    #     state = torch.tensor(state, dtype=torch.float32).unsqueeze(0).to(device)
    #     next_state = torch.tensor(next_state, dtype=torch.float32).unsqueeze(0).to(device)
    #     self.nn.update(state, action, reward, next_state)
    ...
    def replay(self, batch_size):
        ...
        if not samples:  # If we got empty lists, return early
            return

        states, actions, rewards, next_states = zip(*samples)

        states = np.array([self.calculate_z_scores(state) for state in states])
        next_states = np.array([self.calculate_z_scores(state) for state in next_states])
        rewards = np.array([self.reward_normalizer.normalize(r) for r in rewards])
        ...
        current_q_values = self.nn.online_network(states).gather(1, actions.unsqueeze(1))
        next_actions = self.nn.online_network(next_states).max(1)[1].unsqueeze(1)
        next_q_values = self.nn.target_network(next_states).gather(1, next_actions).squeeze(1)
        expected_q_values = rewards + (self.nn.online_network.discount_rate * next_q_values)

        loss = (current_q_values.squeeze() - expected_q_values).pow(2) * weights
        priorities = loss + 1e-5  # Add a small constant to avoid zero priority
        loss = loss.mean()

        self.nn.online_network.optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.nn.online_network.parameters(), max_norm=1.0)
        self.nn.online_network.optimizer.step()
        ...
        if hasattr(self, 'total_steps') and hasattr(self, 'target_update_frequency'):
            if self.total_steps % self.target_update_frequency == 0:
                self.nn.update_target_network()

    def save_nn_params(self, period):
        params = {
            "weights": self.nn.online_network.layers[0].weight.cpu().detach().numpy(),
            "bias": self.nn.online_network.fc.bias.cpu().detach().numpy(),
            "discount_rate": self.nn.online_network.discount_rate,
            "learning_rate": self.nn.online_network.learning_rate
        }
    ...
    # def scale_to_11(self, state):
    #     stored_states = self.memory.get_all_states()
    #     if len(stored_states) > 1:
    #         min_vals = np.min(stored_states, axis=0)
    #         max_vals = np.max(stored_states, axis=0)
    #         scaled = (state - min_vals) / (max_vals - min_vals + 1e-8)
    #         return 2 * scaled - 1
    #     else:
    #         return state
    ...
    def save_nn_params(self, period):
        params = {
            "weights": self.nn.online_network.fc.weight.detach().numpy(),
            "bias": self.nn.online_network.fc.bias.detach().numpy(),
            "discount_rate": self.nn.online_network.discount_rate,
            "learning_rate": self.nn.online_network.learning_rate
        }
        if f"agent_{self.id}_nn_params" not in self.model.data_dict:
    ...
    def learn(self, transition):
        ...
        if len(self.memory) >= self.replay_memory_length:
            self.replay(batch_size=min(self.replay_memory_length, len(self.memory)))
    ...
    def getPatchUtility(self, patch):
        ...
        if self.basic or self.arbitrageur or self.rlearner:
            ...
            if patch.good == "sugar":
                new_utility = (self.wealth_by_good["sugar"] + (patch.Q / self.model.consumption_rate["sugar"])) ** \
                              self.model.consumption_rate["sugar"] \
                ...
                new_utility = (self.wealth_by_good["sugar"]) ** self.model.consumption_rate["sugar"] \
                              * (self.wealth_by_good["water"] + (patch.Q / self.model.consumption_rate["water"])) ** \
                              self.model.consumption_rate["water"]
            else:
                # print(new_utility)
                new_utility = 0
    ...
    def move(self):
        ...
        def moveToMaxEmptyPatch(curr_row, curr_col,
            ...
            def basicMove(max_patch):
                ...
                max_good = np.random.choice(max_patches)
        ...
        np.random.shuffle(empty_patches)
    ...
    def trade(self):
            ...
            if self.basic or self.arbitrageur or self.rlearner:
                if self.target == "water":
                    ...
                    if price >= 1:
                        ...
                        valid_trade = getattr(self, "sugar") > 1 and getattr(partner,
                                                                             "water") > 1 / price and no_crossover
                else:
                    self_not_crossover = self.prospective_MRS(price) > self.reservation_ratio
                    partner_not_crossover = partner.prospective_MRS(price) < partner.reservation_ratio
                    if partner.optimizer:
                    ...
                    if price >= 1:
                        ...
                        valid_trade = getattr(partner, "sugar") > 1 and getattr(self,
                                                                                "water") > 1 / price and no_crossover
            elif self.optimizer:
                if self.target == "water":
        ...
        def executeTrade(partner, price):
            ...
            if not self.optimizer and not partner.optimizer:
                ...
                if self.MRS > partner.MRS:  # self is relatively richer in water than partner
                    while (still_can_trade(
                            price)):  # > 0 and partner.prospective_MRS(price) < price < self.prospective_MRS(price)):
                    ...
                    while (still_can_trade(
                            price)):  # > 0 and self.prospective_MRS(price) < price < partner.prospective_MRS(price)):
            ...
            for attr in rlearner_attributes:
                if hasattr(partner, attr):
                    if attr == "nn":
                        ...
                        ordered_traits["discount_rate"] = partner.nn.online_network.discount_rate
                        ordered_traits["learning_rate"] = partner.nn.online_network.learning_rate
                    ordered_traits[attr] = copy.deepcopy(getattr(partner, attr))

            if agent.model.genetic:
                for attr, val in ordered_traits.items():
                    if np.random.random() <= agent.model.cross_over_rate:
        ...
        neighbor_patches = [(self.row + i, self.col + j)
                            for i in self.model.nav_dict[1] if 0 <= self.row + i < 50
                            for j in self.model.nav_dict[1][i] if 0 <= self.col + j < 50]
        np.random.shuffle(neighbor_patches)
